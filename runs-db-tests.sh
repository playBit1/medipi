...
# Create the database test script
...
async function main() {
  try {
    ...
    patientsWithDispensers.forEach(patient => {
      console.log(`Patient: ${patient.name} | Dispenser: ${patient.dispenser ? patient.dispenser.serialNumber : 'None'}`)
    })
    ...
    dispensersWithChambers.forEach(dispenser => {
      console.log(`Dispenser ${dispenser.serialNumber} has ${dispenser.chambers.length} chambers:`)
    ...
    recentLogs.forEach(log => {
      console.log(
        `${log.timestamp.toLocaleString()}: ${log.status} - Patient: ${log.schedule.patient.name}, ` +
    ...
    if (fullSchedule) {
      console.log(`Schedule for ${fullSchedule.patient.name} at ${fullSchedule.time}:00 has ${fullSchedule.chambers.length} chambers:`)
      fullSchedule.chambers.forEach(content => {
        console.log(`- Chamber ${content.chamber.chamberNumber}: ${content.medication.name}, Dosage: ${content.dosageAmount}`)
      })
    } else {
    ...
    rfidTags.forEach(rfid => {
      console.log(
        `RFID: ${rfid.rfidTag} | Type: ${rfid.type} | Dispenser: ${rfid.dispenser.serialNumber} | ` +
          `Patient: ${rfid.dispenser.patient ? rfid.dispenser.patient.name : 'None'}`
      )
    })
...
# Update the package.json to add the test script
if grep -q '"test:db"' package.json; then
  echo "Test script already exists in package.json"
else
  sed -i.bak 's/"scripts": {/"scripts": {\n    "test:db": "tsx tests\/database.ts",/g' package.json && rm package.json.bak
  echo "Added test:db script to package.json"
fi
